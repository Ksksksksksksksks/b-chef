...
# бот для приготовления еды
# составим список блюд которые поддерживает бот: яичница, варить макароны/рис/гречку, варить яйца, жарить мясо и тд
# пользователь заходит, мы его приветствуем, предлагаем выбрать из 2х версий общения: суровый рамси или милая бабушка
# потом юзеру предлагаем выбрать любое блюдо из доступного списка
# юзер выбирае рецепт, мы закрепляем за ним выбранный рецепт, пскидываем ему пошаговые действия, которые закреплены за блюдом в json с рецептами
# json c рецептами так же будем хранить в проекте, сделай пока для 2х блюд  яичница и варить макароны
# и теперь строим диалог с пользователем в формате он кидает фото/видео-кружок, мы обрабатываем, логики для обратботки пока нет, поэтому сделай просто заглушку: 
# для видео это ответ "Спасибо за видео! У тебя хорошо получается, продолжай в том же духе"
# для фото это ответ "Спасибо за фото! У тебя хорошо получается, продолжай в том же духе"
# пользователю всегда доступны кнопки (завершить готовку, посмотреть рецепт, приготовить новый рецепт)